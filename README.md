...
Cookie 默认共用「查成分」插件的 `data/cha_chengfen/bilibili_cookies.txt`（相对于机器人根目录）。每次开始统计都重新读取文件，因此「查成分」刷新 Cookie 后，下次查询会自动使用新值，无需重启。文件内容为原始 Cookie 请求头字符串；文件存在但为空、格式异常或不可读时会明确报错，不把内容打印到日志。
