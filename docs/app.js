...
let current = null, controller = null, history = [];
try {
...
function renderHistory() {
  ...
  for (const data of history) {
    ...
    button.addEventListener('click',() => {current = data; $('media-input').value = `md${data.mediaId}`; render(); status('已加载本机缓存；点击开始统计可重新采集。');});
...
function renderTrend(allRows, filteredRows) {
  ...
  const min = flat.reduce((v,p) => Math.min(v,p.time),Infinity), max = flat.reduce((v,p) => Math.max(v,p.time),0);
  ...
  const svg = svgEl('svg',{viewBox:'0 0 720 200',role:'img','aria-label':'累计平均评分趋势，蓝色为过滤后，灰色为全部样本。下方可展开数据表。'});
  const x = t => max === min ? 375 : 36 + (t-min)/(max-min)*660;
  const y = value => 160 - value/10*140;
  [2,4,6,8,10].forEach(value => {svg.append(svgEl('line',{x1:36,x2:696,y1:y(value),y2:y(value),stroke:'#e5eaf1'}),svgEl('text',{x:18,y:y(value)+4,'text-anchor':'middle'},value));});
  sets.forEach((points,index) => {
    const valid = points.filter(p => p.average != null), color = index ? '#1261d9' : '#bbc6d7';
    ...
    valid.forEach(p => {const point = svgEl('circle',{cx:x(p.time),cy:y(p.average),r:3,fill:color}); point.append(svgEl('title',{},`${index?'过滤后':'全部样本'} ${date(p.time)}：${p.average.toFixed(2)}`));svg.append(point);});
  });
  svg.append(svgEl('text',{x:36,y:190},date(min)),svgEl('text',{x:696,y:190,'text-anchor':'end'},date(max))); $('trend').append(svg);
...
function render() {
  ...
  $('result-meta').textContent = `数据时间 ${new Date(current.timestamp).toLocaleString('zh-CN')} · 切换等级即时重算`;
  ...
  renderDistribution(all,filtered); renderTrend(allRows,filteredRows);
  ...
  $('probability').textContent = `原脚本模型估计（±0.1）：全部样本 ${percent(probability(allRows,current.officialCount))}；过滤后 ${percent(probability(filteredRows,current.officialCount))}。样本少于 2 条或总体人数未知时不计算。`;
...
async function runAnalysis(input) {
  ...
  try {
    ...
    current = validateDataset(result); render(); status('统计完成。可切换用户等级比较结果。'); remember(current);
...
$('import-file').addEventListener('change',async event => {
  ...
  try {
    ...
    current = data; $('media-input').value = `md${current.mediaId}`; render(); status('已导入数据，未发起网络请求。'); remember(current);
